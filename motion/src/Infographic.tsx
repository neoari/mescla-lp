...
export const FPS = 30;
export const getPhase = (frame: number) => Math.min(2, Math.max(0, Math.floor(frame / 150)));
...
const Paper: React.FC<{children: React.ReactNode; x: number; y: number; w?: number; h?: number; color?: string; angle?: number; depth?: number; opacity?: number}> = ({children,x,y,w=290,h=245,color=white,angle=0,depth=0,opacity=1}) =>
  <div style={{position:'absolute',left:x,top:y,width:w,height:h,padding:w<190?20:27,background:color,border:'1px solid #1B193024',borderRadius:12,boxShadow:'0 2px 0 #c9c3cf, 0 5px 0 #d8d2df, 0 26px 40px #1b193019',transform:`perspective(950px) rotateY(${angle}deg) rotateX(8deg) translateZ(${depth}px)`,transformStyle:'preserve-3d',opacity,color:ink}}>{children}</div>;
...
function Creator({frame}:{frame:number}) {
  ...
  return <>
    <div style={{position:'absolute',left:50,top:interpolate(frame,[250,310],[80,42],ease),width:540,padding:'24px 27px',borderRadius:15,background:'#302A42',border:'1px solid #696071',transform:`perspective(1000px) rotateX(${interpolate(frame,[0,170],[14,2],ease)}deg)`,opacity:interpolate(frame,[270,320],[1,.55],ease)}}>
      ...
      <div style={{display:'flex',alignItems:'center',gap:7,height:110,marginTop:22}}>{Array.from({length:39},(_,i)=><span key={i} style={{display:'block',width:6,borderRadius:4,height:18+Math.abs(Math.sin(i*2.3))*65+(frame<270?Math.sin(frame/13+i)*9:0),background:i<interpolate(frame,[40,245],[0,39],ease)?mint:'#71657D'}}/>)}</div>
