...
def home_value():
    rows = [
        ('target', 'Menos tentativa e erro', 'Você escolhe a rotina, configura a IA e testa o que funciona.', 'Escolhemos um primeiro processo com você e testamos com materiais reais do seu negócio.'),
        ('plug', 'Menos trabalho de preparação', 'Você organiza referências, instruções e conexões entre ferramentas.', 'Preparamos contexto, agentes e conexões para as etapas combinadas. Você recebe o trabalho para revisar.'),
        ('route', 'Mais continuidade nas entregas', 'Você ensina a equipe a usar e ajusta o processo quando ele muda.', 'Colocamos a rotina em uso com sua equipe. Suporte e melhorias seguem o acompanhamento contratado.'),
    ]
    body = ''.join(f'<tr role="row"><th scope="row" role="rowheader"><span>{ui_icon(key)}{escape(gain)}</span></th><td role="cell"><span class="comparison-mobile-label" aria-hidden="true">Ao montar por conta própria</span>{escape(alone)}</td><td role="cell"><span class="comparison-mobile-label" aria-hidden="true">Com a Mescla</span>{escape(together)}</td></tr>' for key,gain,alone,together in rows)
    return f'''<section class="section mescla-value" id="possibilidades" aria-labelledby="possibilities-title"><div class="wrap"><div class="section-head"><div><p class="eyebrow">Já tenho uma IA. Por que a Mescla?</p><h2 id="possibilities-title">Mais trabalho entregue.<br>Com a IA preparada<br>para o seu negócio.</h2></div><div><p class="lead">ChatGPT, Claude e Gemini podem fazer parte da solução. A Mescla prepara essas ferramentas para a sua rotina e acompanha as primeiras entregas com você.</p>{tool_marks([('openai','ChatGPT'),('claude','Claude'),('googlegemini','Gemini')], True)}</div></div><table class="value-comparison" role="table" aria-label="Montar uma rotina de IA por conta própria ou com a Mescla"><thead role="rowgroup"><tr role="row"><th scope="col" role="columnheader">O que você ganha</th><th scope="col" role="columnheader">Ao montar por conta própria</th><th scope="col" role="columnheader">Com a implantação da Mescla</th></tr></thead><tbody role="rowgroup">{body}</tbody></table><div class="value-proof"><span class="value-proof-icon">{ui_icon('file-check-2')}</span><p><strong>O ganho precisa aparecer no trabalho.</strong> No primeiro projeto, comparamos tempo gasto, ajustes necessários e entregas aprovadas.</p><a class="text-link" href="#como-funciona">Como começamos {ui_icon('arrow-up-right')}</a></div></div></section>'''
