...
def value_section(d):
    ...
    if slug == 'empreendedores':
        body = '<div class="value-ledger">' + ''.join(f'<article><h3>{esc(title)}</h3><div><span class="value-label">Onde trava</span><p>{esc(before)}</p></div><div><span class="value-label">Com apoio dos agentes</span><p>{esc(after)}</p></div></article>' for title,before,after in cards) + '</div>'
    elif slug == 'creators':
        icons = ['youtube', 'instagram', 'notion']
        body = '<div class="format-shelf">' + ''.join(f'<article>{icon(icons[i])}<span class="format-number">FORMATO / 0{i+1}</span><h3>{esc(title)}</h3><p>{esc(after)}</p><div class="format-origin">A partir do seu conteúdo</div></article>' for i,(title,before,after) in enumerate(cards)) + '</div>'
    elif slug == 'consultorias':
        body = '<div class="consulting-findings">' + ''.join(f'<article><span class="finding-number">0{i+1}</span><div><h3>{esc(title)}</h3><p>{esc(after)}</p></div><aside>{esc(before)}</aside></article>' for i,(title,before,after) in enumerate(cards)) + '</div>'
    elif slug == 'agencias':
        body = '<div class="agency-lanes">' + ''.join(f'<article><div class="lane-title"><span>0{i+1}</span><h3>{esc(title)}</h3></div><p class="lane-friction">{esc(before)}</p><p>{esc(after)}</p></article>' for i,(title,before,after) in enumerate(cards)) + '</div>'
    else:
        body = '<div class="legal-briefs">' + ''.join(f'<article><span class="brief-no">{i+1:02d} / APOIO AO TRABALHO</span><h3>{esc(title)}</h3><p>{esc(after)}</p><div class="legal-duty">Revisão e validação pelo escritório</div></article>' for i,(title,before,after) in enumerate(cards)) + '</div>'
...
def tools_section(d):
    cards = ''.join(f'<article class="stack-tool"><div>{icon(key)}<h3>{escape(name)}</h3></div><p>{escape(text)}</p></article>' for key,name,text in d['tools'])
...
def ai_section(d):
    ...
    return f'''<section class="section ai-section" aria-labelledby="ai-title"><div class="wrap ai-layout"><div class="ai-badges">{badges}<span class="ai-caption">Se a IA já faz parte da sua rotina</span></div><div><p class="eyebrow">O próximo passo é fazer o trabalho continuar</p><h2 id="ai-title">{escape(d['ai_title'])}</h2><p class="lead">{escape(d['ai_text'])}</p><div class="platform-note"><strong>Pessoas e agentes no mesmo ambiente.</strong><p>Podemos trabalhar com Buzz, <a href="https://docs.openclaw.ai/start/teams" target="_blank" rel="noopener noreferrer">OpenClaw</a>, Hyperagent ou Hermes. A escolha considera sua equipe, as tarefas e os acessos necessários.</p></div></div></div></section>'''

def pilot_section(d):
    items = ''.join(f'<li><span aria-hidden="true">0{i+1}</span>{escape(x)}</li>' for i,x in enumerate(d['deliverables']))
